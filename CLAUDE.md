...
`supabase/functions/notify-new-submission/index.ts` is a single Edge Function invoked by two plain SQL triggers (`supabase/migrations/20260802_notify_new_submission_trigger.sql`, one on `enquiries` INSERT, one on `reviews` INSERT via `pg_net.http_post`) — it branches on the `table` field it's sent, and emails `cristina_cristina973@yahoo.com` via Resend either way. **Not** the dashboard's Database Webhooks UI — that failed on this project (`schema "supabase_functions" does not exist`, never provisioned), so it's wired up as a raw trigger instead. Deployed and confirmed firing correctly end-to-end; blocked only on Resend's sandbox sender restriction (can't deliver to `cristina_cristina973@yahoo.com` until a sending domain is verified) — see README for details. Unlike restore-relax, this had to be written fresh since restore-relax's equivalent function isn't in git and can't be copied directly.
