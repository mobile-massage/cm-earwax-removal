...
`supabase/functions/notify-new-submission/index.ts` is a single Edge Function invoked by two plain SQL triggers (`supabase/migrations/20260802_notify_new_submission_trigger.sql`, one on `enquiries` INSERT, one on `reviews` INSERT via `pg_net.http_post`) — it branches on the `table` field it's sent, and emails `cristina_cristina973@yahoo.com` via Resend either way, sending from `enquiries@cmearwaxremoval.co.uk` (verified sending domain in Resend). **Not** the dashboard's Database Webhooks UI — that failed on this project (`schema "supabase_functions" does not exist`, never provisioned), so it's wired up as a raw trigger instead. Deployed and confirmed working end-to-end (`200` from Resend, real email delivered) — see README for the verification query. Unlike restore-relax, this had to be written fresh since restore-relax's equivalent function isn't in git and can't be copied directly.
